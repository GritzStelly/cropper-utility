  ...
  const handleExport = async (finalCrops: Record<string, StoredCrop>) => {
        ...
        if (crop) {
            try {
                ...
                console.error(`Failed to crop ${image.name}:`, error);
                 setErrors(prev => [...prev, { fileName: image.name, message: 'Failed during final crop.'}]);
            }
        }
    }
    ...
    if (croppedImageBlobs.length > 0) {
        try {
            ...
            setErrors(prev => [...prev, { fileName: 'ZIP Archive', message: 'Failed to create downloadable zip file.'}]);
        }
    }
      ...
      case AppState.DONE:
        return (
            <div className="text-center flex flex-col items-center gap-6">
                <Icon icon="check" className="w-24 h-24 text-green-400" />
                <h2 className="text-3xl font-bold text-gray-100">All Done!</h2>
                <p className="text-gray-400 max-w-md">Your images have been successfully cropped and downloaded as a .zip file. You can start over to crop more images.</p>
                <button
                    onClick={handleReset}
